...
public class GItem extends AWidget implements ItemInfo.SpriteOwner, GSprite.Owner, RandomSource {
    ...
    public void wdgmsg(String msg, Object... args) {
        if (msg.equals("iact")) {
            ...
            int modflags = (args.length > 1) ? ((Integer) args[1]) : 0;
            WItem source = (args.length > 2) ? (WItem) args[2] : null; // Extract WItem if provided
            ui.interact(this, c, modflags, source); // Pass WItem to UI
        } else if (msg.equals("transfer")) {
            Coord c = (Coord) args[0];
            int n = (args.length > 1) ? Utils.iv(args[1]) : 1;
            if (n < 0)
                ui.xfer(this, c, Integer.MAX_VALUE);
            else
                ui.xfer(this, c, n);
            ...
            int n = (args.length > 1) ? Utils.iv(args[1]) : 1;
            if (n < 0)
                ui.drop(this, c, Integer.MAX_VALUE);
            else
                ui.drop(this, c, n);
        } else if (msg.equals("take")) {
            ui.take(this, (Coord) args[0]);
        } else if (msg.equals("itemact")) {
            ui.itemact(this, (args.length > 0) ? Utils.iv(args[0]) : 0);
        } else if (msg.equals("ttupdate")) {
            ui.ttupdate(this);
        } else {
