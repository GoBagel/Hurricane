...
public class UI {
    ...
    public class NewWidget implements Runnable, Serializable {
        ...
        public void run() {
            ...
            if(wdg == null) {
                if("sm".equals(typenm) && lastInteractSource != null) {
                    wdg = new FlowerMenu(lastInteractSource, (String[]) cargs);
                            ...
                            for(WItem item : parent.wmap.values()) {
                                for(ItemInfo ii : item.item.info()) {
                                    if(ii instanceof Wear) {
                                        Wear wear = (Wear) ii;
                                        if(OptWnd.preventCutleryFromBreakingCheckBox.a && wear.d == wear.m - 1 && item.item.getres() != null && (!recentlyTakenCutlery.containsKey(item.item.getres().name) || System.currentTimeMillis() - recentlyTakenCutlery.get(item.item.getres().name) > 500)) {
